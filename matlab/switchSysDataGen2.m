...
function [y, u, gt, a, b] = switchSysDataGen2(opt)
...
if nargin == 0
    ...
    den_ord = [2 2 2]; % order for each system, minimum 2
    num_ord = [2 2 2];
...
%             i = mod(i, num_sys)+1;
        ...
        y(:,j+den_ord(i)) =  y(:,j:j+den_ord(i)-1) * a{i}(1:end-1).' + u(:,j:j+num_ord(i)-1) * b{i}.' + noiseLevel * (2*rand-1);
        gt(j) = i;
end
